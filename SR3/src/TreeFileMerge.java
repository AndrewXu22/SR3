...
public class TreeFileMerge  {
	
	
	static String filePath1;
	//public static int numSpl;
	
	public TreeFileMerge(String filepath) throws IOException {
        FileMerge.filePath1 = filepath;
	...
	public void fileMerger() throws IOException {
		String FILE_NAME = FileMerge.filePath1 + File.pathSeparator + "original_data.txt";
		...
		File directory = new File(FileMerge.filePath1);
				...
				if (part_map.higherEntry(m.getKey()) != null) {
					...
					if (part_map.size() > 1 && m.getKey() == j  ) { // here to check if this two parts are consecutive
						...
						for (Entry<Integer, String> n:merge_map.entrySet()) {						
							list.add(new File(FileMerge.filePath1 + File.pathSeparator + n.getValue()));														
							mergeEnd = n.getKey();
						}
						...
						String File_Merged = FileMerge.filePath1 + File.pathSeparator + "merged_begin_"+ mergeBegin +"_to_"+ mergeEnd;
